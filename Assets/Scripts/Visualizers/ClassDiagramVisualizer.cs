...
{
    public class ClassDiagramVisualizer : IGraphVisualizer
    {
        public void RenderGraph(GraphMetadata graph, GameObject container, List<NodeData> nodes, List<EdgeData> edges)
        {
            ...
            foreach (var node in nodes)
            {
                ...
                if (nodeObj.TryGetComponent<Renderer>(out var rend))
                    rend.material.color = new Color(0.98f, 0.96f, 0.82f);
            ...
            Debug.Log($"Rendered class diagram for {graph?.Key ?? "?"} inside {container.name}");
        }
